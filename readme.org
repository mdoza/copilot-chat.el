...
* News
** Incompatibility with copilot-emacs/copilot.el
The [[https://github.com/copilot-emacs/copilot.el][copilot-emacs/copilot.el]] package (MELPA package: ~copilot~) now ships its own
~copilot-chat.el~ that provides the same ~copilot-chat~ Emacs feature.  The two
packages *cannot be loaded in the same Emacs session* because they define
conflicting symbols in the ~copilot-chat~ namespace.

- If you use ~copilot-emacs/copilot.el~ for inline completions *and* want
  ~copilot-chat.el~'s richer chat interface, you must choose one or the other.
- To keep using this package, uninstall the MELPA ~copilot~ package
  (~M-x package-delete RET copilot~).
- To switch to the built-in chat in ~copilot-emacs/copilot.el~, uninstall
  this package (~M-x package-delete RET copilot-chat~).
...
Use ~mcp-hub~ to check that your servers work. While in a ~copilot-chat~ instance, use ~copilot-chat-set-mcp-servers~ to enable or disable servers for the instance. Not all models are able to use this feature, so choose wisely.
...
** Backend
~copilot-chat-backend~ can only be set to ~'curl~.
...
*** Backend
- ~copilot-chat-backend~ - Backend to use for API calls. Can be ~'curl~.
...
- ~copilot-chat-git-wait-message-format~ - Format string for the message displayed while generating a commit message.
- ~copilot-chat-git-regenerate-wait-message-format~ - Format string for the message displayed while regenerating a commit message.
